...
collect_live_info(){
    ...
    hostnamectl > "$OUTPATH"/LiveInfo/SystemInfo/Hostname.txt
    uname -a > "$OUTPATH"/LiveInfo/SystemInfo/System.txt
    cat /proc/cpuinfo > "$OUTPATH"/LiveInfo/SystemInfo/CPU.txt
    dmesg > "$OUTPATH"/LiveInfo/SystemInfo/KernelMessageBuffer.txt
    dmidecode > "$OUTPATH"/LiveInfo/SystemInfo/DMITable.txt
    lshw > "$OUTPATH"/LiveInfo/SystemInfo/Hardware.txt
    mount > "$OUTPATH"/LiveInfo/SystemInfo/MountedFs.txt
    ps auxw > "$OUTPATH"/LiveInfo/SystemInfo/Processes.txt
    ...
    arp -e > "$OUTPATH"/LiveInfo/SystemInfo/ARP.txt
    ip a > "$OUTPATH"/LiveInfo/SystemInfo/Interfaces.txt
    ip --details route show table all > "$OUTPATH"/LiveInfo/SystemInfo/NetworkRoutes.txt
    iptables -L -v -n > "$OUTPATH"/LiveInfo/SystemInfo/IptablesRules.txt
    nft list ruleset > "$OUTPATH"/LiveInfo/SystemInfo/NftablesRules.txt
    lsof -nPl > "$OUTPATH"/LiveInfo/SystemInfo/NetworkFiles.txt 2>/dev/null
    ss -anepo > "$OUTPATH"/LiveInfo/SystemInfo/NetworkConnections.txt
    if [ "$operating_system" == 1 ]; then
     dpkg -l > "$OUTPATH"/LiveInfo/SystemInfo/InstalledPackages.txt;
    elif [ "$operating_system" == 2 ]; then
     rpm -q -a > "$OUTPATH"/LiveInfo/SystemInfo/InstalledPackages.txt;
    fi
    if [[ $(which pip) != "" ]]; then
     pip list -v > "$OUTPATH"/LiveInfo/SystemInfo/PythonPackages.txt 2>/dev/null
    fi
    df -h > "$OUTPATH"/LiveInfo/SystemInfo/DiskSpace.txt
    free -htw > "$OUTPATH"/LiveInfo/SystemInfo/RAM.txt
    lsmod > "$OUTPATH"/LiveInfo/SystemInfo/LoadedModules.txt
    echo "Host information from live system collected"

    # Logins
    last -a -F > "$OUTPATH"/LiveInfo/Logins/Logins.txt
    lastb -a -F > "$OUTPATH"/LiveInfo/Logins/UnsuccessfulLogins.txt
    lastlog > "$OUTPATH"/LiveInfo/Logins/LastLoginByUser.txt
    who -a > "$OUTPATH"/LiveInfo/Logins/AllLoggedUsers.txt
    w > "$OUTPATH"/LiveInfo/Logins/LoggedUsersWithProcs.txt
    echo "Login data collected"
    pwck > "$OUTPATH"/LiveInfo/PasswordVerification.txt
    
    sysctl -a > "$OUTPATH"/LiveInfo/KernelParameters.txt
    
    # Services
    service --status-all > "$OUTPATH"/LiveInfo/Services/ServicesStatus.txt
    systemctl list-units --no-pager --all > "$OUTPATH"/LiveInfo/Services/SystemdSystemUnits.txt
    systemctl list-timers --all > "$OUTPATH"/LiveInfo/Services/SystemdTimers.txt

    # Time
    timedatectl status > "$OUTPATH"/LiveInfo/Time/TimeSettings.txt
    uptime -s > "$OUTPATH"/LiveInfo/Time/Uptime.txt
